...
DISPLAY = True  # Want to see a graph of the estiamte?

Xk = np.matrix([1.0, 0.5]).T  # state vector [position, velocity]
Pk = np.matrix([[1.0, 0.0], [0.0, 1.0]])  # covariance matrix
delta_t = 1
...
for i in range(5):
    ...
    # -------- Measurments ---------
    Hk = np.matrix([[1.0, 0.0], [0.0, 1.0]])  # maps the sensor reading values to
    ...
    # #Sensor Values
    Zk = np.mean([i**2, 1.5])  # Mean value of sensors
    # # Covariance matrix of measurments
    Rk = np.matrix([[1.0, 0.0], [0.0, 1.0]])
    # -------- /Measurments ---------

    # # -------- Update Step ---------
    intuition = np.dot(np.dot(Hk, Pk), Hk.T) + Rk
    # # print intuition

    K = np.dot(np.dot(Pk, Hk.T), inv(intuition))
    ...
    if DISPLAY:
        delta = 1.25
        x = np.arange(-3.0, 50.0, delta)
        y = np.arange(-3.0, 50.0, delta)
        X, Y = np.meshgrid(x, y)
